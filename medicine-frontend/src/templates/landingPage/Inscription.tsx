...
import { registerPatientType } from "@/templates/landingPage/actions/types.ts";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function Inscription() {
  const [informationInscription, setInformationInscription] =
    useState<registerPatientType>({
      password: "",
      email: "",
      nom: "",
      prenom: "",
  ...
  const validateFields = () => {
    ...
    if (
      !informationInscription.email.match(
        /^[\w.-]+@[a-zA-Z\d.-]+\.[a-zA-Z]{2,}$/
      )
    ) {
    ...
    if (
      !informationInscription.password.match(
        /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{8,}$/
      )
    ) {
      newErrors.password =
  ...
  const handleInputChange = (
    ...
    if (typeof e === "string") {
      ...
      setInformationInscription((prev) => ({
        ...prev,
  ...
  const register = useMutation({
    mutationFn: async (informationInscription: registerPatientType) =>
      registerPatient(informationInscription),
    onSuccess: () => {
          ...
          <div className="w-full max-w-md p-6 bg-white rounded-lg shadow-xl">
            ...
            <Tabs>
              <form onSubmit={handleSubmit} className="space-y-4">
                <SelectMedecin
                  informationInscription={informationInscription}

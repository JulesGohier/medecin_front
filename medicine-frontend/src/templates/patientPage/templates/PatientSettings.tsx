...
export const PatientSettings = () => {
  ...
  useEffect(() => {
    ...
    delete patientData.rendez_vous;
  ...
  const mutation = useMutation({
    mutationFn: async (modifiedFields: Patient) => {
      return await updateInformationPatient(
        modifiedFields,
        patientData.num_secu_sociale
      );
    },
    onSuccess: async () => {
  ...
  const validateFields = () => {
    ...
    if (
      formData.email &&
      !formData.email.match(/^[\w.-]+@[a-zA-Z\d.-]+\.[a-zA-Z]{2,}$/)
    ) {
    ...
    if (
      formData.num_secu_sociale &&
      !formData.num_secu_sociale.match(/^\d{15}$/)
    ) {
      newErrors.num_secu_sociale =
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (Object.keys(modifiedFields).length !== 0) {
      if (modifiedFields.email || modifiedFields.password) {
  ...
  return (
    <DashboardWrapper user={patientData}>
      {redirectToLogin && <RedirectionConnexion isOpen={redirectToLogin} />}
      <form onSubmit={handleSubmit} className="flex flex-col space-y-4 p-6">
        ...
        <div className="flex flex-col">
          <label htmlFor="prenom" className="font-medium text-sm">
            Prénom
          </label>
          <Input
            id="prenom"
        ...
        <div className="flex flex-col">
          <label htmlFor="num_secu_sociale" className="font-medium text-sm">
          ...
          <Input
            ...
            type="text"
          />
          {errors.num_secu_sociale && (
            <p className="text-red-500 text-sm">{errors.num_secu_sociale}</p>
          )}
        </div>
        ...
        <div className="flex flex-col">
          <label htmlFor="medecin_perso" className="font-medium text-sm">
        ...
        <div className="flex flex-col">
          <label htmlFor="num_tel" className="font-medium text-sm">
            Téléphone
          </label>
          <Input
            id="num_tel"
          ...
          />
          {errors.num_tel && (
            <p className="text-red-500 text-sm">{errors.num_tel}</p>
          )}
        </div>
        ...
        <div className="flex flex-col">
          <label htmlFor="date_naissance" className="font-medium text-sm">
            Date de naissance
          </label>
          <Input
            id="date_naissance"
        ...
        <Button
          type="submit"
          className={
            "bg-red-500 hover:bg-red-600 w-full flex items-center gap-4"
          }
          disabled={
            Object.keys(modifiedFields).length === 0 || mutation.isPending
          }
        >
          {mutation.isPending
